...
public class QuestionHandlePresenter extends ParentPresenter<QuestionHandleMvpView> {
    ...
    private List<DUMedia> xczpFiles;
    private List<DUMedia> bpzpFiles;
    ...
    @Inject
    public QuestionHandlePresenter(DataManager dataManager, ConfigHelper configHelper
        ...
        xczpFiles = new ArrayList<>();
        bpzpFiles = new ArrayList<>();
    ...
    QuestionHandleEntity handleOrderEntity;
    DUMyTask duMyTask;

    public void commitMyTaskInfo(DUMyTask duMyTask, QuestionHandleEntity handleOrderEntity) {
...
//            getMvpView().onError("param is null");
        ...
        mSubscription.add(mDataManager.getMediaList(taskId, taskType,
                ...
                .subscribe(new Subscriber<List<DUMedia>>() {
                    @Override
                    public void onCompleted() {
                        loadXczpImages(taskId, taskType, taskState);
...
//                        getMvpView().onLoadImages(duMediaList);
     ...
     * @param taskState
     */
    public void loadXczpImages(final String taskId, final int taskType, final int taskState) {
        if (TextUtil.isNullOrEmpty(taskId)) {
//            getMvpView().onError("param is null");
            LogUtil.e(TAG, "param is null");
            return;
        ...
        LogUtil.i(TAG, "loadxczpImages");
        mSubscription.add(mDataManager.getMediaList(taskId, taskType,
                taskState, DUMedia.FILE_TYPE_PICTURE_XCHJZP)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<List<DUMedia>>() {
                    @Override
                    public void onCompleted() {
                        loadBpzpImages(taskId, taskType, taskState);
                        LogUtil.i(TAG, "loadxczpImages onCompleted");
                    }

                    @Override
                    public void onError(Throwable e) {
                        LogUtil.e(TAG, "loadxczpImages onError " + e.getMessage());
//                        getMvpView().onError(e.getMessage());
                    }

                    @Override
                    public void onNext(List<DUMedia> duMediaList) {
                        LogUtil.i(TAG, "loadxczpImages onNext" + duMediaList.size());
...
//                        getMvpView().onLoadImages(duMediaList);
     ...
     * @param taskState
     */
    public void loadBpzpImages(final String taskId, final int taskType, final int taskState) {
        if (TextUtil.isNullOrEmpty(taskId)) {
//            getMvpView().onError("param is null");
            LogUtil.e(TAG, "param is null");
            return;
        ...
        LogUtil.i(TAG, "loadxczpImages");
        mSubscription.add(mDataManager.getMediaList(taskId, taskType,
                taskState, DUMedia.FILE_TYPE_PICTURE_BPZP)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<List<DUMedia>>() {
                    @Override
...
//                .writeTimeOut(3*60*1000)
    ...
    private void commitInfo(final String msg, final String taskId, final int taskType, final int taskState) {
        ...
        if (xczpFiles.size() == 1) {
            if (xczpFiles.get(0) != null) {
                xczp4 = timeDir + "/" + xczpFiles.get(0).getFileName();
            }
        } else if (xczpFiles.size() == 2) {
            if (xczpFiles.get(0) != null) {
                xczp4 = timeDir + "/" + xczpFiles.get(0).getFileName();
            }
            if (xczpFiles.get(1) != null) {
                xczp5 = timeDir + "/" + xczpFiles.get(1).getFileName();
            }
        } else {
            if (xczpFiles.get(0) != null) {
                xczp4 = timeDir + "/" + xczpFiles.get(0).getFileName();
            ...
            if (xczpFiles.get(2) != null) {
                xczp6 = timeDir + "/" + xczpFiles.get(2).getFileName();
            }
        }
        ...
        if (bpzpFiles.size() == 1) {
            if (bpzpFiles.get(0) != null) {
                bpzp7 = timeDir + "/" + bpzpFiles.get(0).getFileName();
            }
        } else if (bpzpFiles.size() == 2) {
            if (bpzpFiles.get(0) != null) {
                bpzp7 = timeDir + "/" + bpzpFiles.get(0).getFileName();
            }
            if (bpzpFiles.get(1) != null) {
                bpzp8 = timeDir + "/" + bpzpFiles.get(1).getFileName();
            }
        } else {
            if (bpzpFiles.get(0) != null) {
                bpzp7 = timeDir + "/" + bpzpFiles.get(0).getFileName();
            ...
            if (bpzpFiles.get(2) != null) {
                bpzp9 = timeDir + "/" + bpzpFiles.get(2).getFileName();
            }
        }
        ...
        String bssyy = "";
        if("情况不属实".equals(handleOrderEntity.getXcshqk())){
            bssyy = handleOrderEntity.getShbz();
        }

        PostRequest request = EasyHttp
                .post(URL.YiWenSBBack)
                .params("albh", duMyTask.getCaseId())
//                .params("xchd", duMyTask.getFaId())
//                .params("zhbh", duMyTask.getAcctId())
//                .params("cjyid", mDataManager.getAccount())
                .params("xcshjg", handleOrderEntity.getXcshqk())
                .params("ywsbcljg", "")
                .params("bssyy", bssyy)
                .params("shbz", handleOrderEntity.getShbz())
            ...
            public void onSuccess(String s) {
                ...
                if (uploadSuccess.getState() == 0) {
                    ...
                    if (handleOrderEntity != null) {
                        GreenDaoUtils.getDaoSession(MainApplication.getInstance()).getQuestionHandleEntityDao()
