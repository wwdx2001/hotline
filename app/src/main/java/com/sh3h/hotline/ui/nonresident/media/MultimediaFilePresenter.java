 ...
 *
 */
public class MultimediaFilePresenter extends ParentPresenter<MultimediaFileMvpView> {
    private static final String TAG = "MultimediaFilePresenter";
    ...
    @Inject
    public MultimediaFilePresenter(DataManager dataManager,
    ...
    public void saveNewImage(final String taskId, final int taskType, final int taskState,
                             final String fileName, File filePath, boolean isFromAlbum, Bitmap bitmap, boolean isHighQulity) {
        if (/*TextUtil.isNullOrEmpty(taskId)
        ...
        if (isFromAlbum && bitmap != null) {//来自本地相册
            if (!mFileHelper.saveJPG(bitmap, filePath.getPath(), isHighQulity)) {
        ...
        mSubscription.add(mDataManager.compressImageAndAddStamp(fileName, filePath, isHighQulity)
     ...
     */
    public void saveNewImage(final String taskId, final int taskType, final int taskState,
                             final String fileName, File filePath, boolean isFromAlbum, Bitmap bitmap, boolean isHighQulity, String address) {
        if (/*TextUtil.isNullOrEmpty(taskId)
                || */TextUtil.isNullOrEmpty(fileName)
                || (filePath == null)) {
            getMvpView().onError("param is null");
        ...
        LogUtil.i(TAG, "saveNewImage");
        mSubscription.add(mDataManager.compressImageAndAddStamp(fileName, filePath, isHighQulity, address)
                .concatMap(new Func1<Boolean, Observable<Boolean>>() {
                    @Override
                    public Observable<Boolean> call(Boolean aBoolean) {
                        DUMedia duMedia = new DUMedia(
