...
public class UserApiService extends BaseApiService {
	...
	public LoginResultEntity Login(LoginInfoEntity info) throws ApiException {
		...
		try {
			...
			JSONObject logininfoObject = new JSONObject();
			logininfoObject.put("Account", info.getLoginfo().getAccount());
			logininfoObject.put("Password", info.getLoginfo().getPassword());
			jsonObject.putOpt("loginfo", logininfoObject);

			Log.e("LoginPresenter", jsonObject.toString());
			resp = this.callJSONObject(UserApiService.METHOD_LOGIN, jsonObject);
