package com.sh3h.dataprovider.data.entity.newentity;

import org.greenrobot.greendao.annotation.Entity;
import org.greenrobot.greendao.annotation.Id;
import org.greenrobot.greendao.annotation.Generated;

@Entity
public class OverrateReceiptHandleEntity {

    @Id(autoincrement = true)
    private Long ID;

    private String albh;
    private String pch;
    private String xh;
    private String yhh;

    private String psfs;
    private String ghxdh;
    private String sfsd;
    private String wqsyy;
    private String qsr;
    private String sjrsf;
    private String fdqkbz;
    private String xxbg;

    private String zhbh;
    private String xzq;
    private String ssdm;
    private String khlx;
    private String tyshxydm;
    private String khmc;

    private String lxr;
    private String lxfs;
    private String yjdz;
    private String bz;
    private String youbian;

    @Generated(hash = 2124063736)
    public OverrateReceiptHandleEntity(Long ID, String albh, String pch, String xh,
            String yhh, String psfs, String ghxdh, String sfsd, String wqsyy,
            String qsr, String sjrsf, String fdqkbz, String xxbg, String zhbh,
            String xzq, String ssdm, String khlx, String tyshxydm, String khmc,
            String lxr, String lxfs, String yjdz, String bz, String youbian) {
        this.ID = ID;
        this.albh = albh;
        this.pch = pch;
        this.xh = xh;
        this.yhh = yhh;
        this.psfs = psfs;
        this.ghxdh = ghxdh;
        this.sfsd = sfsd;
        this.wqsyy = wqsyy;
        this.qsr = qsr;
        this.sjrsf = sjrsf;
        this.fdqkbz = fdqkbz;
        this.xxbg = xxbg;
        this.zhbh = zhbh;
        this.xzq = xzq;
        this.ssdm = ssdm;
        this.khlx = khlx;
        this.tyshxydm = tyshxydm;
        this.khmc = khmc;
        this.lxr = lxr;
        this.lxfs = lxfs;
        this.yjdz = yjdz;
        this.bz = bz;
        this.youbian = youbian;
    }
    @Generated(hash = 1265210146)
    public OverrateReceiptHandleEntity() {
    }
    public Long getID() {
        return this.ID;
    }
    public void setID(Long ID) {
        this.ID = ID;
    }
    public String getAlbh() {
        return this.albh;
    }
    public void setAlbh(String albh) {
        this.albh = albh;
    }
    public String getPch() {
        return this.pch;
    }
    public void setPch(String pch) {
        this.pch = pch;
    }
    public String getXh() {
        return this.xh;
    }
    public void setXh(String xh) {
        this.xh = xh;
    }
    public String getYhh() {
        return this.yhh;
    }
    public void setYhh(String yhh) {
        this.yhh = yhh;
    }
    public String getPsfs() {
        return this.psfs;
    }
    public void setPsfs(String psfs) {
        this.psfs = psfs;
    }
    public String getGhxdh() {
        return this.ghxdh;
    }
    public void setGhxdh(String ghxdh) {
        this.ghxdh = ghxdh;
    }
    public String getSfsd() {
        return this.sfsd;
    }
    public void setSfsd(String sfsd) {
        this.sfsd = sfsd;
    }
    public String getWqsyy() {
        return this.wqsyy;
    }
    public void setWqsyy(String wqsyy) {
        this.wqsyy = wqsyy;
    }
    public String getQsr() {
        return this.qsr;
    }
    public void setQsr(String qsr) {
        this.qsr = qsr;
    }
    public String getSjrsf() {
        return this.sjrsf;
    }
    public void setSjrsf(String sjrsf) {
        this.sjrsf = sjrsf;
    }
    public String getFdqkbz() {
        return this.fdqkbz;
    }
    public void setFdqkbz(String fdqkbz) {
        this.fdqkbz = fdqkbz;
    }
    public String getXxbg() {
        return this.xxbg;
    }
    public void setXxbg(String xxbg) {
        this.xxbg = xxbg;
    }
    public String getZhbh() {
        return this.zhbh;
    }
    public void setZhbh(String zhbh) {
        this.zhbh = zhbh;
    }
    public String getXzq() {
        return this.xzq;
    }
    public void setXzq(String xzq) {
        this.xzq = xzq;
    }
    public String getSsdm() {
        return this.ssdm;
    }
    public void setSsdm(String ssdm) {
        this.ssdm = ssdm;
    }
    public String getKhlx() {
        return this.khlx;
    }
    public void setKhlx(String khlx) {
        this.khlx = khlx;
    }
    public String getTyshxydm() {
        return this.tyshxydm;
    }
    public void setTyshxydm(String tyshxydm) {
        this.tyshxydm = tyshxydm;
    }
    public String getKhmc() {
        return this.khmc;
    }
    public void setKhmc(String khmc) {
        this.khmc = khmc;
    }
    public String getLxr() {
        return this.lxr;
    }
    public void setLxr(String lxr) {
        this.lxr = lxr;
    }
    public String getLxfs() {
        return this.lxfs;
    }
    public void setLxfs(String lxfs) {
        this.lxfs = lxfs;
    }
    public String getYjdz() {
        return this.yjdz;
    }
    public void setYjdz(String yjdz) {
        this.yjdz = yjdz;
    }
    public String getBz() {
        return this.bz;
    }
    public void setBz(String bz) {
        this.bz = bz;
    }
    public String getYoubian() {
        return this.youbian;
    }
    public void setYoubian(String youbian) {
        this.youbian = youbian;
    }
}
